...
impl TaskControlBlock {
    ...
    pub fn spawn(self: &Arc<Self>, elf_data: &[u8]) -> isize {
        ...
        let kernel_stack = kstack_alloc();
        let kernel_stack_top = kernel_stack.get_top();
        // tcb中除了parent外和new中相同，parent参考fork
        ...
        // 当前进程：添加儿子、修改状态、加入taskmanager
        let mut parent_inner = self.inner_exclusive_access();
        parent_inner.children.push(task_control_block.clone());
        parent_inner.task_status = TaskStatus::Ready;
        add_task(current_task().unwrap());
        // prepare TrapContext in user space
        let task_inner = task_control_block.inner_exclusive_access();
        ...
        *trap_cx = TrapContext::app_init_context(
        ...
        // 修改processor相关信息
        set_current(Some(task_control_block.clone()));
        // drop exclusive_access并切换
        let current_task_cx_ptr = &mut parent_inner.task_cx as *mut TaskContext;
